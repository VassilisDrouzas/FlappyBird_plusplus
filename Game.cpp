...
void Game::spawnPipe(game_difficulty difficulty)
{
	...
	//AddPipes();									//×ñçóéìåýåé óôï íá åíôÜîïõìå ôá áíôéêåßìåíá pipes óôï vector
}
/*
void Game::AddPipes() {								//ÐñïóèÞêç óôïé÷åßùí ôýðïõ Enemy óôï vector pipes
...
void Game::deletePipe()
{
	/*
	for (auto iter : pipes) {
															// ÄéáãñáöÞ óôïé÷åßùí ôýðïõ Enemy áðü ôï vector pipes ìå iterator.×ñçóßìåõå óå ðåñßðôùóç ðïõ èÝëáìå íá áðïèçêåýóïõìå ôá pipes óå Ýíá vector.
...
void Game::updateLevelScreen1(game_difficulty difficulty) {
	...
	if (difficulty == NORMAL) {
		...
		/*
		for (auto iter : pipes) {				//Óõíôïìüôåñç ëýóç ðïõ ðñïóðÜèçóá ìå iterator óå vector. ÐñïÝêõøáí ðåñßåñãá áðïôåëÝóìáôá,ãéá áõôü êáé äåí ôçí ÷ñçóéìïðïßçóá ôåëéêÜ..
			if (iter) {
				(*iter).update_hard();
			}
		}*/
